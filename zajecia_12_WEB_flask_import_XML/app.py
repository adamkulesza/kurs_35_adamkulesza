...
# Funkcja parsująca dane z XML
def parse_xml_metadata(C0128M01):
    tree = ET.parse(C0128M01)
    root = tree.getroot()
    ns = {'nrt': 'urn:schemas-professionalDisc:nonRealTimeMeta:ver.2.20'}

    video_format = root.find('.//nrt:VideoFormat/nrt:VideoFrame', ns)
    audio_format = root.find('.//nrt:AudioFormat', ns)
    device = root.find('.//nrt:Device', ns)
    lens = root.find('.//nrt:Lens', ns)

    return {
        "videoCodec": video_format.attrib.get('videoCodec', '') if video_format is not None else '',
...
def show_users():
    metadata = parse_xml_metadata("C0128M02.xml")  # Podaj nazwę/ścieżkę do pliku XML
    ...
    if request.method == "POST":
        new_user = {
            ...
            "videoCodec": request.form.get("videoCodec"),
            "audioChannels": request.form.get("audioChannels"),
            "deviceModel": request.form.get("deviceModel"),
            "lensModel": request.form.get("lensModel")
        }
        users.append(new_user)

    return render_template("users.html", users=users, metadata=metadata)
